...
namespace Tp.Integration.Plugin.Common.Tests.Concurrency.Utils
{
    ...
    internal class Chess
    {
        ...
        private static Process RunInChess(MethodBase methodInfo, ChessOptions options)
        {
            ...
            string chessCommandLine = string.Format("{0}.dll /testclass:{1} /ia:{2} /detectraces", testAssemblyName, testClassName,
                domainAssemblyName);
            if (options.Debug)
            {
            ...
            if (!File.Exists(@"Concurrency\chess\mchess.exe"))
            {
                NUnit.Framework.Assert.Ignore(
                    "Chess.exe is not present in Libs folder. Test will not run. Upload chess from http://research.microsoft.com/en-us/projects/chess/download.aspx");
            }
