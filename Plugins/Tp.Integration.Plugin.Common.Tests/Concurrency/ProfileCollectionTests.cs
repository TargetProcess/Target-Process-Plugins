...
namespace Tp.Integration.Plugin.Common.Tests.Concurrency
{
    ...
    public class ProfileCollectionTests : DomainObjectConcurrencyTest
    {
        ...
        public void ReadWrite()
        {
            ...
            AccountCollection.GetOrCreate(ACCOUNT_NAME)
                .Profiles.Count()
                .Should(Be.EqualTo(2), "AccountCollection.GetOrCreate(ACCOUNT_NAME).Profiles.Count().Should(Be.EqualTo(2))");
        }
        ...
        public void WriteOnceReadTwice()
        {
            ...
            }, () =>
            {
                ...
                if (Interlocked.Increment(ref afterProfileAdded) == 2)
                {
                    ...
                    latestAccountVersion.Profiles.Count()
                        .Should(Be.EqualTo(1), "latestAccountVersion.Profiles.Count().Should(Be.EqualTo(1))");
                    if (sameVersion)
                    {
            ...
            AccountCollection.GetOrCreate(account1Name)
                .Profiles.Count()
                .Should(Be.EqualTo(1), "AccountCollection.GetOrCreate(account1Name).Profiles.Count().Should(Be.EqualTo(1))");
        }
