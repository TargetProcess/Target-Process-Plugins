...
namespace Tp.Integration.Plugin.Common.Tests.Common
{
    ...
    public class ProfileStorageSqlPersisterSpecs : SqlPersisterSpecBase
    {
        ...
        public void ShouldStoreAndRetrieveNamedRecordForProfileByName()
        {
            ...
            var profileStorage = profileStorages[1];
            var retrieved =
                _persister.FindBy(_profile.Id, new TypeNameWithoutVersion(profileStorage.ValueKey), profileStorage.Name).Single();
            retrieved.Should(Be.EqualTo(profileStorage).Using<ProfileStorage>(Compare),
                "retrieved.Should(Be.EqualTo(profileStorage).Using<ProfileStorage>(Compare))");
        }
        ...
        public void ShouldStoreAndRetrieveNamedRecordForProfileByNameAndValue()
        {
            ...
            var profileStorage = profileStorages[1];
            var retrieved = _persister.FindBy(profileStorage.Name, _profile.Id, new TypeNameWithoutVersion(profileStorage.ValueKey),
                profileStorage.GetValue());
            retrieved.Should(Be.EqualTo(profileStorage).Using<ProfileStorage>(Compare),
                "retrieved.Should(Be.EqualTo(profileStorage).Using<ProfileStorage>(Compare))");

            _persister.FindBy(profileStorage.Name, _profile.Id, new TypeNameWithoutVersion(profileStorage.ValueKey),
                profileStorage.GetValue() + "123").Should(
                Be.Null,
                "_persister.FindBy(profileStorage.Name, _profile.Id, new TypeNameWithoutVersion(profileStorage.ValueKey), profileStorage.GetValue() + \"123\").Should(Be.Null)");
        }
        ...
        public void ShouldDetectContainmentByNameAndValue()
        {
            ...
            var profileStorage = profileStorages[1];
            _persister.Contains(profileStorage.Name, _profile.Id, new TypeNameWithoutVersion(profileStorage.ValueKey),
                    profileStorage.GetValue())
                .Should(Be.True,
            ...
            _persister.Contains(profileStorage.Name, _profile.Id, new TypeNameWithoutVersion(profileStorages[0].ValueKey),
                profileStorage.GetValue()).Should(
                Be.False,
                "_persister.Contains(profileStorage.Name, _profile.Id, new TypeNameWithoutVersion(profileStorages[0].ValueKey), profileStorage.GetValue()).Should(Be.False)");
        }
        ...
        public void ShouldDeleteByName()
        {
            ...
            _persister.Contains(profileStorage.Name, _profile.Id, new TypeNameWithoutVersion(profileStorage.ValueKey),
                profileStorage.GetValue()).Should(
                Be.False,
                "_persister.Contains(profileStorage.Name, _profile.Id, new TypeNameWithoutVersion(profileStorage.ValueKey), profileStorage.GetValue()).Should(Be.False)");
        }
