...
namespace Tp.Integration.Plugin.Common.Tests.Common
{
    ...
    public class ProfileStorageSpecs
    {
        ...
        {
            SetCurrentPluginContext(accountName, profileName);
            StorageRepository.Get<StringValue>()
                .First()
                .Value.Should(Be.EqualTo(stringValue), "StorageRepository.Get<StringValue>().First().Value.Should(Be.EqualTo(stringValue))");
        }
        ...
        public void ProfileStorageShouldContainListOfStrings(string profileName, string accountName, string[] stringValues)
        {
            ...
            list.Select(x => x.Value)
                .ToArray()
                .Should(Be.EquivalentTo(stringValues), "list.Select(x => x.Value).ToArray().Should(Be.EquivalentTo(stringValues))");
        }
