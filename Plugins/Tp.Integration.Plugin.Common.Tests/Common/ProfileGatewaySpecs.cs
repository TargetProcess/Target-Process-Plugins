    ...
    public class ProfileGatewaySpecs : SqlPersisterSpecBase
    {
            ...
            string accountName)
        {
            messagesSentLocally.Should(Be.EquivalentTo(expectedMessageNames),
                "messagesSentLocally.Should(Be.EquivalentTo(expectedMessageNames))");
            Bus.GetOutProfileName()
                .Should(Be.EqualTo((ProfileName) profileName), "Bus.GetOutProfileName().Should(Be.EqualTo((ProfileName) profileName))");
            Bus.GetOutAccountName()
                .Should(Be.EqualTo(new AccountName(accountName)), "Bus.GetOutAccountName().Should(Be.EqualTo(new AccountName(accountName)))");
        }
