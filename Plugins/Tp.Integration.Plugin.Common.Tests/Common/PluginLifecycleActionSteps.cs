    ...
    public class PluginLifecycleActionSteps
    {
        ...
        public void AccountShouldHaveProfiles(string accountName, string[] profileNames)
        {
            ...
            account.Profiles.Select(x => x.Name.Value)
                .ToArray()
                .Should(Is.EquivalentTo(profileNames),
                    "account.Profiles.Select(x => x.Name.Value).ToArray().Should(Is.EquivalentTo(profileNames))");
        ...
        {
            _sentMessages.OfType<PluginAccountMessageSerialized>()
                .Count()
                .Should(Is.EqualTo(messageCount),
        ...
        {
            _sentMessages.OfType<PluginAccountMessageSerialized>()
                .ForEach(x => x.GetAccounts().Should(Be.Empty, "x.GetAccounts().Should(Be.Empty)"));
        }
        ...
        public void ShouldPublishPluginInfoMessageWithProfiles(string accountName, string[] profileNames)
        {
            ...
            pluginLifecycleMessage.GetAccounts().First(x => x.Name == accountName).PluginProfiles.Select(x => x.Name.Value).
                ToArray().Should(
                    ...
                    "pluginLifecycleMessage.GetAccounts().First(x => x.Name == accountName).PluginProfiles.Select(x => x.Name.Value).ToArray().Should(Is.EquivalentTo(profileNames))");
        }
        ...
        public void PluginInfoMessageShouldBePublished()
        {
            ...
            messages.FirstOrDefault(x => x.Info.Name == ObjectFactory.GetInstance<IPluginContext>().PluginName).Should(
                Be.Not.Null,
                "messages.FirstOrDefault(x => x.Info.Name == ObjectFactory.GetInstance<IPluginContext>().PluginName).Should(Be.Not.Null)");
        }
        ...
        public void ShouldPublishPluginInfoMessage(string accountName, string profileName)
        {
            ...
            message.GetAccounts()
                .First()
                .PluginProfiles.SingleOrDefault(x => x.Name == profileName)
                .Should(Is.Not.Null,
                    "message.GetAccounts().First().PluginProfiles.SingleOrDefault(x => x.Name == profileName).Should(Is.Not.Null)");

            ObjectFactory.GetInstance<PluginContextMock>().AccountName = accountName;
            ObjectFactory.GetInstance<IProfileCollection>()
                .SingleOrDefault(x => x.Name == profileName)
                .Should(Is.Not.Null,
        ...
        public void ScriptShouldBeSentToTargetProcess(string mashupName, string scriptContent, string placeholderName)
        {
            ...
            var sentScripts = bus.GetCallsMadeOn<IBus, PluginMashupMessage>(x => x.SendToUi()).ToArray();
            var sentMashup = sentScripts.First().First(x => x.MashupName == mashupName);
            sentMashup.PluginMashupScripts.Select(x => x.ScriptContent)
                .ToArray()
                .Should(Is.EquivalentTo(new[] { scriptContent }),
        ...
        {
            File.Exists(AttachmentFolder.GetAttachmentFileFullPath(new FileId(Guid.Parse(fileName))))
                .Should(Be.True, "File.Exists(AttachmentFolder.GetAttachmentFileFullPath(new FileId(Guid.Parse(fileName)))).Should(Be.True)");
        }
    }
