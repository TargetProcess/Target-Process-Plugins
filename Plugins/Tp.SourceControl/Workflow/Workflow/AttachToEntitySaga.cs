...
{
    public class AttachToEntitySaga
        : TpSaga<AttachToEntitySagaData>,
          ...
          IHandleMessages<TargetProcessExceptionThrownMessage>
    {
        ...
        public void Handle(AssignRevisionToEntityAction message)
        {
            ...
            _logger.InfoFormat("Assigning revision to entity. Revision ID: {0}; Assignable ID: {1}", message.Dto.SourceControlID,
                message.EntityId);
        ...
        {
            _logger.Error(
                string.Format("Failed to attach revision to entity. Revision ID: {0}; Entity ID: {1}", Data.RevisionDto.SourceControlID,
                    Data.EntityId), message.GetException());
