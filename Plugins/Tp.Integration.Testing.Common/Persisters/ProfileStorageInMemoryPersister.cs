...
namespace Tp.Integration.Testing.Common.Persisters
{
    public class ProfileStorageInMemoryPersister : IProfileStoragePersister
    {
        ...
        {
            _profileStorage.RemoveAll(
                x => storageNames.Select(s => s.Value).Contains(x.Name) && x.ProfileId == profileId.Value && x.ValueKey == key.Value);
        }
        ...
        public IEnumerable<ProfileStorage> FindBy(ProfileId profileId, TypeNameWithoutVersion key, params StorageName[] storageNames)
        {
            return
                FindBy(profileId)
                    .Where(
                        x => x.ValueKey == key.Value && storageNames.Select(s => GetLowerOrNull(s.Value)).Contains(GetLowerOrNull(x.Name)));
        }
