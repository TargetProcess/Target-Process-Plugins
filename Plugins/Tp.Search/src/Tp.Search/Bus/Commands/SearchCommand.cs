...
namespace Tp.Search.Bus.Commands
{
    class SearchCommand : IPluginCommand
    {
        ...
        public SearchCommand(QueryRunner queryRunner, IProfile profile, DocumentIndexRebuilder documentIndexRebuilder,
        ...
        private IndexProgressData CreateIndexingProgress(QueryResult result)
        {
            ...
            double completeInPercents = allEntities != 0
                ? (double) ((result.LastIndexedEntityId + result.LastIndexedCommentId + result.LastIndexedTestStepId) * 100) / allEntities
                : 0;
