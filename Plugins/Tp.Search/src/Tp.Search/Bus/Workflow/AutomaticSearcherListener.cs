...
namespace Tp.Search.Bus.Workflow
{
    class AutomaticSearcherListener : IHandleMessages<TickMessage>
    {
        ...
        public AutomaticSearcherListener(IPluginContext pluginContext, IProfileReadonly profile,
            IDocumentIndexProvider documentIndexProvider, IActivityLogger logger)
        {
        ...
        public void Handle(TickMessage message)
        {
            ...
            {
                _documentIndexProvider.ShutdownDocumentIndexesIfRunning(_pluginContext,
                    new DocumentIndexShutdownSetup(forceShutdown: false, cleanStorage: false), _logger);
            }
        }
    }
}
