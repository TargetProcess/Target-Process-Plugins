...
namespace Tp.Search.Model.Query
{
    internal class QueryPlanBuilder
    {
        ...
        public QueryPlanBuilder(IPluginContext pluginContext, IProfileReadonly profile, IDocumentIndexProvider documentIndexProvider,
            IEntityTypeProvider entityTypeProvider, IIndexDataFactory indexDataFactory, ContextQueryPlanBuilder contextQueryPlanBuilder)
        {
        ...
        private Maybe<QueryPlan> CreateCommentPlan(QueryData data, ParsedQuery parsedQuery)
        {
            ...
            var contextPlan = _contextQueryPlanBuilder.Build(data, DocumentIndexTypeToken.CommentProject,
                DocumentIndexTypeToken.CommentSquad, DocumentIndexTypeToken.CommentEntityType);
        ...
        private Maybe<QueryPlan> CreateTestStepPlan(QueryData data, ParsedQuery parsedQuery)
        {
            ...
            var projectContextPlan = _contextQueryPlanBuilder.BuildProjectContextPlan(data.ProjectIds, data.IncludeNoProject,
        ...
        private Maybe<QueryPlan> CreateEntityPlan(QueryData queryData, ParsedQuery parsedQuery)
        {
            ...
            var contextPlan = _contextQueryPlanBuilder.Build(queryData, DocumentIndexTypeToken.EntityProject,
                DocumentIndexTypeToken.EntitySquad, DocumentIndexTypeToken.EntityType);
            if (!contextPlan.HasValue)
            {
                return Maybe.Nothing;
            }
        ...
        private Maybe<QueryPlan> CreateEntityTypePlan(int? entityTypeId)
        {
            ...
            return entityTypeFinders.BuildExecutionPlan(new ParsedQuery(words: string.Format("+{0}", entityTypeName.ToLower())),
