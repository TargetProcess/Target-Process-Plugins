...
namespace Tp.Search.Model.Document
{
    class DocumentIndexProvider : IDocumentIndexProvider
    {
        ...
        public DocumentIndexProvider(IActivityLoggerFactory loggerFactory, DocumentIndexSetup documentIndexSetup,
            DocumentIndexMetadata documentIndexMetadata, DocumentIndexOptimizeHintFactory optimizeHintFactory)
        {
        ...
        {
            get { return _documentIndexMetadata.DocumentIndexTypes; }
        }
        ...
        public IEnumerable<IDocumentIndex> GetOrCreateDocumentIndexes(IPluginContext context,
            params DocumentIndexTypeToken[] indexTypeTokens)
        {
        ...
        private IDocumentIndex DoGetOrCreateDocumentIndex(IPluginContext context, DocumentIndexTypeToken documentIndexTypeToken)
        {
            ...
            var index = storage.GetOrAdd(context.AccountName.Value, key => Lazy.Create(() =>
            {
                ...
                var otherVersions =
                    documentIndex.Type.GetVersions(context.AccountName, _documentIndexSetup).Except(new[] { documentIndex.Type.Version });
        ...
        private IDocumentIndex CreateDocumentIndex(IPluginContext context, DocumentIndexTypeToken documentIndexTypeToken,
            int? version = null)
        {
            ...
            {
                charactersIndexType = _documentIndexMetadata.GetDocumentIndexType(documentIndexTypeToken,
                    DocumentIndexDataTypeToken.Characters);
            }
            ...
            monitor = new DocumentIndexMonitor(charactersIndexType ?? digitsIndexType, () =>
            {
                if (charactersIndexType != null)
                {
                    return digitsIndexType != null
                        ? (IDocumentIndex)
                        new DocumentIndex(charactersIndexType, digitsIndexType, context, detachIndex, _documentIndexSetup, _loggerFactory,
                            _optimizeHintFactory)
                        : new DocumentIndexTyped(charactersIndexType, context, detachIndex, _documentIndexSetup, _loggerFactory,
                            _optimizeHintFactory);
                }
