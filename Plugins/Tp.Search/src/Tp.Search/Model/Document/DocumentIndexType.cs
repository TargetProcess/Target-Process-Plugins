...
namespace Tp.Search.Model.Document
{
    [DocumentIndex(
         ...
         new[]
         {
             typeof(GeneralIndexAttribute), typeof(AssignableIndexAttribute), typeof(TestStepIndexAttribute),
             typeof(ImpedimentIndexAttribute), typeof(CommentIndexAttribute)
         },
         documentAttributeTypes:
         new[] { typeof(GeneralDocumentAttribute), typeof(AssignableDocumentAttribute), typeof(ImpedimentDocumentAttribute) },
    ...
    class DocumentIndexType
    {
        ...
        public DocumentIndexType(DocumentIndexTypeToken typeToken, DocumentIndexDataTypeToken dataTypeToken, string fileName,
            IEnumerable<Enum> indexFields, IEnumerable<Enum> documentFields, int version,
            DocumentIndexDataTypeService documentIndexDataTypeService, IFileService fileService)
        {
        ...
        {
            return new DocumentIndexType(_typeToken, _dataTypeToken, _fileName, _indexFields, _documentFields, version,
                _documentIndexDataTypeService, _fileService);
        }
        ...
        {
            string basePath = string.IsNullOrEmpty(documentIndexSetup.IndexPath)
                ? string.Format("{0}\\{1}", IndexFileRootPath, "index")
                : documentIndexSetup.IndexPath;
