    ...
    class EnumServices
    {
        ...
        public static IDictionary<TEnum, Attribute> Load<TEnum>(Type attributeType)
        {
            return typeof(TEnum).GetFields(BindingFlags.Public | BindingFlags.Static)
                .Join(Enum.GetValues(typeof(TEnum)).Cast<TEnum>(), f => f.Name, e => e.ToString(),
                    (f, e) => new { Field = f, EnumValue = e })
                .Select(
                    p =>
                    {
        ...
        public static Exception CreateUnexpectedEnumError<TEnum>(TEnum value) where TEnum : struct
        {
            return new InvalidOperationException(string.Format("Wrong enum {0} value: {1}", typeof(TEnum), value));
        }
    }
}
