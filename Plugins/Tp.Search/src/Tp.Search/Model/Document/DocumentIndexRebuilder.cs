...
namespace Tp.Search.Model.Document
{
    class DocumentIndexRebuilder
    {
        ...
        public DocumentIndexRebuilder(IDocumentIndexProvider documentIndexProvider, DocumentIndexSetup documentIndexSetup, ITpBus bus,
            IPluginContext pluginContext, IPluginMetadata pluginMetadata, IProfileCollection profileCollection, IProfile profile,
        ...
        public bool RebuildIfNeeded(bool shouldRebuildIfNoProfile = false)
        {
            ...
            var shouldRebuildIfNoAnyFile =
                _documentIndexProvider.DocumentIndexTypes.Any(
                    type => type.GetVersions(_pluginContext.AccountName, _documentIndexSetup).Empty());
            if (shouldRebuildIfNoActualVersionForAnyFile || shouldRebuildIfNoAnyFile)
            {
                _logger.InfoFormat("Going to start rebuild becuase of {0}",
                    shouldRebuildIfNoActualVersionForAnyFile ? "no actual version of index file" : "no actual versions of index files");
                DoRebuild();
                return true;
            }
            return false;
