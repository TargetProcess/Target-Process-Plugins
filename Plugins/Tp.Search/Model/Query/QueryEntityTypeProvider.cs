...
namespace Tp.Search.Model.Query
{
	...
	class QueryEntityTypeProvider : IQueryResultFactory, IEntityTypeProvider
	{
		...
		{
			return _entityTypeIds.Contains(entityTypeId.GetValueOrDefault());
		}
		...
		public string QueryableEntityTypesIdSqlString
		{
			get
			{
				return string.Join(",", _entityTypeIds.Where(e => e != COMMENT_TYPE_ID).Select(entityTypeId => entityTypeId.ToString(CultureInfo.InvariantCulture)).ToArray());
			}
		}
