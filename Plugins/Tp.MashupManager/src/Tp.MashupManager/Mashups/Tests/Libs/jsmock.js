...
JSMock = {
    extend: function(object) {
        ...
        object.createMock = function(objectToMock) {
            return mockControl.createMock(objectToMock)
        };
        object.resetMocks = function() {
            mockControl.reset()
        };
        object.verifyMocks = function() {
            mockControl.verify()
        };
...
MockControl.prototype = {
    ...
    createMock: function(objectToMock) {
        var mock = { calls: [], expects: function() {
    ...
    andReturn: function(returnValue) {
        ...
        this.__lastMock.calls[this.__lastCallName].push(function() {
            return returnValue;
        });
    },
...
Similar.to = function(simular) {
    return new Similar(simular);
};
...
ArgumentMatcher = {
    ...
    __delegateMatching: function(expected, actual) {
        ...
        else if (expected.constructor == Like) {
            return this.__matchLike(expected.expected, actual);
        }
        else if (expected.constructor == Similar) {
            return this.__matchSimilar(expected.expected, actual);
        }
    ...
    __getProperties:function(object) {
        var properties = [];
    ...
    __matchLike:function(expected, actual) {

        var expectedProperties = this.__getProperties(expected);
        var actualProperties = this.__getProperties(actual);
        var ret = this.__matchArrays(expectedProperties, actualProperties);
        ...
        if (ret) {
            ...
            for (var key in expected) {
                ...
                var expKey = expected[key];
                var actKey = actual[key];
                if (expKey && actKey && expKey instanceof Object && actKey instanceof Object) {
                    expectedValues.push(true);
                    actualValues.push(this.__delegateMatching(expKey, actKey));
                }
                else {
                    expectedValues.push(expKey);
                    actualValues.push(actKey);
                }
            }

            ret = this.__matchArrays(expectedValues, actualValues);
        }

        return ret;
    },

    __matchSimilar:function(expected, actual) {
