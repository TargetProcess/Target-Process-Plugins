...
namespace Tp.MashupManager.Tests.MashupLibrary
{
    ...
    public class LibraryCommandSpecs : MashupManagerTestBase
    {
        ...
        {
            MockedRepositories.SelectMany(x => x.RefreshedLibraries)
                .Select(p => p.Name)
                .Should(Be.EquivalentTo(libraries),
        ...
        public void CheckReturnedSources(string sourceName, string packages)
        {
            ...
            commandResult
                ...
                .Should(Be.EquivalentTo(packages.Split(',').Select(p => p.Trim())),
                    "commandResult.Single(r => r.Name == sourceName.Trim()).Packages.Select(p => p.Name).Should(Be.EquivalentTo(packages.Split(',').Select(p => p.Trim())))");
        }
        ...
        {
            _response.ResponseData.Deserialize<PackageDetailedDto>()
                .ReadmeMarkdown.Should(Be.EqualTo(markdownContent),
