...
namespace Tp.Integration.Plugin.TestRunImport.Ftps
{
    public sealed class FtpsClient : IDisposable
    {
        ...
        readonly Stack<string> _currDirStack = new Stack<string>();
        ...
        /// </summary>
        public X509Certificate RemoteCertificate => ctrlSslStream?.RemoteCertificate;
        ...
        public X509Certificate LocalCertificate => ctrlSslStream?.LocalCertificate;
        ...
        /// <param name="remoteFileName"></param>
        /// <returns></returns>
        internal FtpStream GetFile(string remoteFileName)
        {
            SetupDataConnection();
        ...
        /// <returns>Transferred bytes count.</returns>
        internal ulong GetFile(string remoteFileName, string localFileName, FileTransferCallback transferCallback)
        {
            ulong totalBytes = 0;
        ...
        /// <remarks>
        /// </remarks>
        internal void GetFiles(string remoteDirectoryName, string localDirectoryName,
        ...
        /// <param name="remoteFileName"></param>
        /// <returns></returns>
        internal FtpStream PutFile(string remoteFileName)
        {
            SetupDataConnection();
        ...
        /// <param name="transferCallback"></param>
        /// <returns>Transferred bytes count.</returns>
        internal ulong PutFile(string localFileName, string remoteFileName, FileTransferCallback transferCallback)
        {
        ...
        /// <param name="recursive"></param>
        /// <param name="transferCallback"></param>
        internal void PutFiles(string localDirectoryName, string remoteDirectoryName,
            string filePattern, EPatternStyle patternStyle, bool recursive,
            FileTransferCallback transferCallback)
        {
            Regex regex = null;
        ...
        internal FtpStream AppendFile(string remoteFileName)
        {
            SetupDataConnection();
        ...
        /// <param name="remoteFileName"></param>
        /// <param name="transferCallback"></param>
        internal ulong AppendFile(string localFileName, string remoteFileName, FileTransferCallback transferCallback)
        {
            using (Stream s = AppendFile(remoteFileName))
                return SendFile(localFileName, remoteFileName, s, transferCallback);
        }

        internal FtpStream PutUniqueFile(out string remoteFileName)
        {
            SetupDataConnection();
        ...
        /// <param name="remoteFileName"></param>
        /// <param name="transferCallback"></param>
        internal ulong PutUniqueFile(string localFileName, out string remoteFileName, FileTransferCallback transferCallback)
        {
        ...
        private void SetupCtrlStreamReaderAndWriter(Stream s)
        {
            ...
            ctrlSw = new StreamWriter(s, encoding) { NewLine = "\r\n" };
        }
        ...
        private void SetupPassiveDataConnection(IPEndPoint dataEndPoint)
        {
            ...
            var addr = useCtrlEndPointAddressForData ? (ctrlClient.Client.RemoteEndPoint as IPEndPoint).Address : dataEndPoint.Address;
        ...
        private SslStream CreateSSlStream(Stream s, bool leaveInnerStreamOpen)
        {
            ...
            sslStream.AuthenticateAsClient(hostname, clientCertColl, SslProtocols.Ssl3 | SslProtocols.Tls/*SslProtocols.Default*/, sslCheckCertRevocation);
        ...
        private FTPReply HandleCmd(string command, bool waitForAnswer = true)
        {
            CheckConnection();
            ...
            LogCommand?.Invoke(this, new LogCommandEventArgs(command));
        ...
        private void PortCmd()
        {
            ...
            string portStr = $"{addr[0]},{addr[1]},{addr[2]},{addr[3]},{port / 256},{port % 256}";
