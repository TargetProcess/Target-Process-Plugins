...
{
    public class CreateTestPlanRunSaga
        : TpSaga<CreateTestPlanRunSagaData>,
          ...
          IHandleMessages<TestCaseRunQueryResult>
    {
        ...
        public void Handle(TestPlanRunCreatedMessage message)
        {
            if (message.Dto.TestPlanRunID.HasValue)
            {
                ...
                Log()
                    .InfoFormat("Start getting test case runs for test plan run #{0} - {1}", message.Dto.TestPlanRunID.Value.ToString(),
                        message.Dto.Name);
        ...
        public void Handle(TestCaseRunUpdatedMessage message)
        {
            if (message.Dto != null && message.Dto.TestPlanRunID == Data.TestPlanRunId)
            {
                ...
                if (testCaseTestPlanDto != null)
                {
                    ...
                    {
                        TestCaseField.LastFailureComment, TestCaseField.LastRunDate, TestCaseField.LastStatus, TestCaseField.LastRunStatus
                    };
...
#pragma warning restore 612
        ...
        private void UpdateTestPlanRunsIfNecessary()
        {
            ...
            Log()
                .InfoFormat("Finished getting test case runs for test plan run #{0}. The number of test case runs retrieved is {1}",
                    Data.TestPlanRunId, Data.TestCaseRunsRetrievedCount);
