    ...
    public abstract class ImportResultsTestRunImportActionSteps
    {
        ...
        public void BeforeScenario()
        {
            ObjectFactory.Configure(
                x =>
                    x.For<TransportMock>()
                        .Use(TransportMock.CreateWithoutStructureMapClear(typeof(TestRunImportPluginProfile).Assembly)));
            ...
            Context.Transport.On<TestCaseTestPlanQuery>(cmd => cmd.TestPlanId == 101).Reply(
                x =>
                    _testCaseTestPlanDtos.Select(
                            testCaseTestPlanDto =>
                                new TestCaseTestPlanQueryResult
                                {
                                    Dtos = new[] { testCaseTestPlanDto },
        ...
        public void TestPlanRunCreatedMessageShouldBeSentFromTargetProcess()
        {
            ...
            testPlanRun.Dto.CommentOnChangingState.Should(
                ...
                "testPlanRun.Dto.CommentOnChangingState.Should(Be.EqualTo(string.Format(\"State is changed by '{{i}}' plugin\", Context.CurrentProfile.Name.Value)))");
        ...
        public void TestCaseRunQueryResultMessageShouldBeSentFromTargetProcess(string[] testCaseNames)
        {
            ...
            foreach (var dto in testCaseNames.Select(FindTestCaseTestPlanDtoByName))
            {
                ...
                caseRunQueryResults.FirstOrDefault(x => x.Dtos.First().TestCaseTestPlanID == dto.TestCaseTestPlanID).Should(
                    Be.Not.Null,
                    "caseRunQueryResults.FirstOrDefault(x => x.Dtos.First().TestCaseTestPlanID == dto.TestCaseTestPlanID).Should(Be.Not.Null)");
            }
        }
        ...
        public void TestCaseRunUpdatedMessageShouldBeSentFromTargetProcess(string[] testCaseNames)
        {
            ...
            {
                dto.Should(Be.Not.Null, "dto.Should(Be.Not.Null)");
                caseRunUpdatedMessages.FirstOrDefault(x => x.Dto.TestCaseTestPlanID == dto.TestCaseTestPlanID)
                    .Should(Be.Not.Null,
        ...
        public void UpdateCommandMessageForTestCaseDtoShouldBeSentFromTargetProcess(string[] testCaseNames)
        {
            ...
            {
                dto.Should(Be.Not.Null, "dto.Should(Be.Not.Null)");
                updateTestCaseCommands.FirstOrDefault(x => x.Dto.ID == dto.TestCaseID)
                    .Should(Be.Not.Null, "updateTestCaseCommands.FirstOrDefault(x => x.Dto.ID == dto.TestCaseID).Should(Be.Not.Null)");
            }
        }
