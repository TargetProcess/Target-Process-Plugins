	...
	public class GitRevisionIdComparerSpecs
	{
		...
		public void ShouldFindMinFromRevision()
		{
			((GitRevisionId) _comparer.FindMinFromRevision(_revisionRanges)).Time.Should(
				Be.EqualTo(((GitRevisionId) _revisionRanges[3].FromChangeset).Time), "((GitRevisionId) _comparer.FindMinFromRevision(_revisionRanges)).Time.Should(Be.EqualTo(((GitRevisionId) _revisionRanges[3].FromChangeset).Time))");
		}
		...
		public void ShouldFindMaxToRevision()
		{
			((GitRevisionId) _comparer.FindMaxToRevision(_revisionRanges)).Time.Should(
				Be.EqualTo(((GitRevisionId) _revisionRanges[2].ToChangeset).Time), "((GitRevisionId) _comparer.FindMaxToRevision(_revisionRanges)).Time.Should(Be.EqualTo(((GitRevisionId) _revisionRanges[2].ToChangeset).Time))");
		}
