    ...
    public static class WorkItemsUsedFields
    {
        ...
        public static List<WorkItemField> Map(WorkItem workItem)
        {
            switch (workItem.Type.Name)
            {
                ...
                default:
                    throw new ArgumentException($"Invalid Work Item Type: {workItem.Type.Name}");
            }
        }
        ...
        private static List<WorkItemField> GetMapping(WorkItem workItem, IEnumerable<string> fields)
        {
            return (from field in fields
                ...
                {
                    Name = field,
                    Type = workItemField.FieldDefinition.FieldType.ToString(),
                    Value = workItemField.Value == null ? string.Empty : workItemField.Value.ToString()
                }).ToList();
        }
    }
}
