...
namespace Tp.Tfs
{
	...
	public class TfsPluginProfile : TfsConnectionSettings, ISynchronizableProfile, IValidatable
	{
		...
		{
			return Regex.IsMatch(Uri, @"^((?#protocol)https?)://((?#domain)[-A-Z0-9.]+)((?#port):\d{1,5})((?#file)/[-A-Z0-9+&@#/%= ~_|!:,.;]+)$", RegexOptions.IgnoreCase);
		}
		...
		private void ValidateUserMapping(PluginProfileErrorCollection errors)
		{
			...
			{
				errors.Add(new PluginProfileError { FieldName = "user-mapping", Message = "Can't map a TFS user to TargetProcess user twice." });
			}
		}
