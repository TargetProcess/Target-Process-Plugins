...
namespace Tp.Subversion.Comments
{
    ...
    public class CommentParserSpecs
    {
        ...
        public void ShouldSupport()
        {
            ...
            var comment =
                $"added headerRenderer to WindowShade control{Environment.NewLine} added fix to allow CanvasButton to work within a Repeater";
            var info = new RevisionInfo { Comment = comment };
            new CommentParser().ParseAssignToEntityAction(info)
                .Should(Be.Empty, "new CommentParser().ParseAssignToEntityAction(info).Should(Be.Empty)");
        }
    }
}
