...
namespace Tp.Subversion.TargetProcessControlByCommentsPostingFeature
{
    ...
    public class ShouldPostCommentSpecs
    {
        ...
        {
            Context.Transport.TpQueue.GetCreatedDtos<CommentDTO>()
                .ToArray()
                .Should(Be.Empty, "Context.Transport.TpQueue.GetCreatedDtos<CommentDTO>().ToArray().Should(Be.Empty)");
        }
        ...
        public void NoAdditionalCommentsShouldBePosted()
        {
            ObjectFactory.GetInstance<TransportMock>()
                .TpQueue.GetCreatedDtos<CommentDTO>()
                .Should(Be.Empty, "ObjectFactory.GetInstance<TransportMock>().TpQueue.GetCreatedDtos<CommentDTO>().Should(Be.Empty)");
        }
