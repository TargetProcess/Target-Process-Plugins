...
namespace Tp.Subversion.TargetProcessControlByCommentsPostingFeature
{
    ...
    public class ShouldProcessEntityIdSpecs
    {
        ...
        public void NoOtherActionsWereTaken()
        {
            ...
            commands.Where(IsNotCreateCommand<RevisionDTO>)
                .Where(IsNotCreateCommand<RevisionAssignableDTO>)
                .Where(x => !(x is RetrieveAllUsersQuery))
                .ToArray()
                .Should(
                    Be.Empty,
                    "commands.Where(IsNotCreateCommand<RevisionDTO>).Where(IsNotCreateCommand<RevisionAssignableDTO>).Where(x => !(x is RetrieveAllUsersQuery)).ToArray().Should(Be.Empty)");
        }
