...
namespace Tp.PopEmailIntegration
{
    ...
    public class TpGeneralUserChangedMessageHandlerSpecs : PopEmailIntegrationContext
    {
        ...
        [Test]
        public void ShouldDeleteUserLiteIfDeleteUserInTP()
        {
            @"Given user with id 1 and first name 'FirstName' stored for profile
        ...
        [Test]
        public void ShouldUpdateUserLiteIfUpdateRequesterInTP()
        {
            @"Given requester with id 1 and first name 'FirstName' stored for profile
				When requester with id 1 is deleted in TP
				Then user with id 1 and type 'Requester' and empty company should be stored for profile"
                .Execute();
        }
        ...
        {
            ObjectFactory.GetInstance<UserRepository>().Add(new UserLite { Id = userId, FirstName = firstName, UserType = UserType.User });
            //Storage.Get<UserLite>().Add(new UserLite {Id = userId, FirstName = firstName});
        }

        [Given("requester with id $requesterId and first name '$firstName' stored for profile")]
        public void StoreRequester(int requesterId, string firstName)
        {
            ObjectFactory.GetInstance<UserRepository>().Add(new UserLite { Id = requesterId, FirstName = firstName, UserType = UserType.Requester });
            //Storage.Get<UserLite>().Add(new UserLite {Id = userId, FirstName = firstName});
        }
        ...
        [When("requester with id $requesterId is deleted in TP")]
        public void DeleteRequesterInTP(int requesterId)
        {
            Transport.HandleMessageFromTp(new RequesterDeletedMessage
            {
        ...
        {
            var user = Storage.Get<UserLite>().First(x => x.Id == userId);
