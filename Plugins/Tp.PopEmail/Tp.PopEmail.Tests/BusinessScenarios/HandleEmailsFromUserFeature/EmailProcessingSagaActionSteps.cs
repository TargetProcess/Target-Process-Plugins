...
namespace Tp.PopEmailIntegration.BusinessScenarios.HandleEmailsFromUserFeature
{
	...
	public class EmailProcessingSagaActionSteps
	{
		...
		{
			var requesters = Context.Storage.Get<UserLite>().Where(x => x.UserType == UserType.Requester);
			var requester = requesters.Where(x => x.Email == requesterMail).FirstOrDefault();
			if (requester == null)
			{
				CreateRequesterWithEmail(requesterMail);
				requester = requesters.Where(x => x.Email == requesterMail).First();
			}
		...
		[Then("user '$userName' should be added as requester and owner to the request")]
		public void AssertRequesterIsOwnerAndRequester(string userName)
		{
			var request =
		...
		{
			return
				Context.Transport.TpQueue.GetMessages<CreateCommand>().Where(x => x.Dto is CommentDTO).First().Dto as CommentDTO;
		}
