    ...
    public class CreateRequestFromMessageSagaSpecs : PopEmailIntegrationContext
    {
        ...
        public void OnBeforeScenario()
        {
            Transport.On<CreateCommand>(x => x.Dto is RequestDTO).Reply(
                message =>
                        new RequestCreatedMessage { Dto = new RequestDTO { ID = 10, Description = ((RequestDTO) message.Dto).Description } });
        ...
        public void MessageIsFromUser(int userId)
        {
            ...
            {
                Dto = new UserDTO { UserID = userId, Email = Guid.NewGuid().ToString() }
            });
        }
        ...
        public void CreateRequestFromMessageCommandReceived(int projectId)
        {
            Transport.HandleLocalMessage(Storage,
                new CreateRequestFromMessageCommand
                {
                    ...
                    Requesters = _requesters.ToArray(),
                    IsPrivate = true
                });
        }
        ...
        {
            Transport.TpQueue.GetMessages<CreateCommand>()
        ...
        {
            RequestDto()
                .SourceType.Should(Be.EqualTo(RequestSourceEnum.Email),
        ...
        public void MessageShouldBeMarkedAsProcessed(int messageId)
        {
            ...
            ObjectFactory.GetInstance<TpInMemorySagaPersister>()
                .Get<ISagaEntity>()
                .Where(x => !(x is NewEmailProfileInitializationSagaData))
                .Count()
                .Should(
        ...
        {
            Transport.TpQueue.GetMessages<CloneAttachmentCommand>()
                .First(x => x.Dto.OriginalFileName == attachmentFileName)
                .Should(Be.Not.Null,
        ...
        {
            var request =
                Transport.TpQueue.GetMessages<UpdateCommand>().Where(x => x.Dto is RequestDTO).Select(x => x.Dto as RequestDTO).First();
