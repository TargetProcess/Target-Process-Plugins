...
my $supportedBugzillaVersion = '5.0';
	...
	foreach my $queryName (@subqueries) {
		...
		if ($query) {
			...
			if ($date) {
				...
				$dd = time2str("%Y-%m-%d %T", str2time($date));
				#die $dd;
				$params->param("chfieldfrom", $dd); #old: date
			...
			my $search = new Bugzilla::Search('fields' => ['bug_id'], 'params' => scalar $params->Vars, 'order' => \@orderstrings, 'allow_unlimited' => 1);

			# Execute the query.
			my ($data, $extra_data) = $search->data;

			foreach my $row (@$data) {
				unshift @bugIds, shift @$row;
			}
		}
		else
		{
...
sub findQuery {
	my $queryName = shift;
	$queryName =~ m/^([a-zA-Z0-9 \._]+)$/ or die "Bad QueryName";
	my $qName= $1;	# Assign untainted data
#	$queryName =~ s/^\s+//;
#	$queryName =~ s/\s+$//;
#	if ($queryName =~ s/\s+$//) {
#		$qName = $1;#
#	}
#	else {
#		die "QueryName is tainted";
#	}

	#get the user id and untaint it
	my $userid = $user->id;
	$userid =~ /^(\d+)$/;

	$dbh = Bugzilla->dbh;
	if($userid) {
		$userqueryid = $dbh->selectrow_array(
...
#	}
	...
	# now find the the userspecific query in the saved searches

	my $query = new Bugzilla::Search::Saved($userqueryid);
	return $query;
}
