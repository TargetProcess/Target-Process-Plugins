    ...
    public class BugsSynchronizer : IHandleMessages<TickMessage>
    {
        ...
        private void ProcessNewlyChanges(DateTime? lastSyncDate)
        {
            ...
            if (lastSyncDate == null || lastSyncronizationDate == null || !lastSyncronizationDate.GetValue().HasValue ||
                lastSyncronizationDate.GetValue().Value.AddMinutes(_storageRepository.GetProfile<BugzillaProfile>().SynchronizationInterval)
                < DateTime.Now)
            {
        ...
        private DateTime? GetSyncDate(DateTime? lastSyncDate)
        {
            ...
            return lastFailedDate != null
                ? lastFailedDate.GetValue()
                : (lastSyncronizationDate != null ? lastSyncronizationDate.GetValue() : lastSyncDate);
        }
