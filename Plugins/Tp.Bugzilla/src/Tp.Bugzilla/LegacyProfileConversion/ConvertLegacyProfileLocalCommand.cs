...
namespace Tp.Bugzilla.LegacyProfileConversion
{
	...
	public class ConvertLegacyProfileHandler : IHandleMessages<ConvertLegacyProfileLocalMessage>
	{
		...
		public void Handle(ConvertLegacyProfileLocalMessage message)
		{
			...
			_log.Info("Bugzilla legacy profile converter parsed parameters");
			...
			_log.Info("Bugzilla legacy profile converter inited db connections and structure map");

			var converter = ObjectFactory.GetInstance<LegacyProfileConvertor>();
			converter.Execute(profileName);
			_log.Info("Bugzilla legacy profile converter executed conversion");
		}

		private static IEnumerable<string> GetParams()
		{
