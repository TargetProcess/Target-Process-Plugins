...
//			                                  	   	});
        ...
        public void CheckStates(string[] states)
        {
            ...
            {
                Storage.Get<EntityStateDTO>()
                    .Select(x => x.Name)
                    .ToArray()
                    .Should(Contains.Item(state), "Storage.Get<EntityStateDTO>().Select(x => x.Name).ToArray().Should(Contains.Item(state))");
            }
        }
        ...
        public void CheckPriorities(string[] priorities)
        {
            ...
            {
                Storage.Get<PriorityDTO>()
                    .Select(x => x.Name)
                    .ToArray()
                    .Should(Contains.Item(priority),
        ...
        public void CheckSeverities(string[] severities)
        {
            ...
            {
                Storage.Get<SeverityDTO>()
                    .Select(x => x.Name)
                    .ToArray()
                    .Should(Contains.Item(severity),
        ...
        public void CheckRoles(string[] roles)
        {
            foreach (var role in roles)
            {
                Storage.Get<RoleDTO>()
                    .Select(x => x.Name)
                    .ToArray()
                    .Should(Contains.Item(role), "Storage.Get<RoleDTO>().Select(x => x.Name).ToArray().Should(Contains.Item(role))");
            }
        }
        ...
        {
            Storage.Get<ProjectDTO>()
                .Select(p => p.Abbreviation)
                .ToArray()
                .Should(Be.EquivalentTo(projects),
        ...
        public void CheckBug(string bugName, string externalId, string projectName)
        {
            ...
            Storage.Get<TargetProcessBugId>(externalId)
                .Single()
                .Value.Should(Be.EqualTo(bugId), "Storage.Get<TargetProcessBugId>(externalId).Single().Value.Should(Be.EqualTo(bugId))");
            Storage.Get<BugzillaBugInfo>(bugId.ToString(CultureInfo.InvariantCulture))
                .Single()
                .Id.Should(Be.EqualTo(externalId),
        ...
        public void CheckComments(string bugName, string[] comments)
        {
            ...
            Storage.Get<CommentDTO>(bugId.ToString(CultureInfo.InvariantCulture))
                ...
                .Should(Be.EquivalentTo(comments),
                    "Storage.Get<CommentDTO>(bugId.ToString(CultureInfo.InvariantCulture)).Select(c => c.Description).ToArray().Should(Be.EquivalentTo(comments))");
        }
        ...
        public void CheckAttachments(string bugName, string[] attachments)
        {
            ...
            Storage.Get<AttachmentDTO>().Where(c => c.GeneralID == bugId)
                ...
                .Should(Be.EquivalentTo(attachments),
                    "Storage.Get<AttachmentDTO>().Where(c => c.GeneralID == bugId).Select(c => c.Description).ToArray().Should(Be.EquivalentTo(attachments))");
        }

        [Then("profile should have $count saved bugs")]
        public void CheckBugsCount(int count)
        {
            Storage.Get<TargetProcessBugId>()
                .Count()
                .Should(Be.EqualTo(count), "Storage.Get<TargetProcessBugId>().Count().Should(Be.EqualTo(count))");
            Storage.Get<BugzillaBugInfo>()
                .Count()
                .Should(Be.EqualTo(count), "Storage.Get<BugzillaBugInfo>().Count().Should(Be.EqualTo(count))");
        }
