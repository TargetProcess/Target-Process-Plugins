...
namespace Tp.Bugzilla.Tests.Synchronization
{
    ...
    public class BugzillaBugInfoStorageSpecs : BugzillaTestBase
    {
        ...
        {
            Context.BugzillaBugInfos.Where(x => x.Id == bugzillaBugId.ToString())
                .Should(Be.Empty, "Context.BugzillaBugInfos.Where(x => x.Id == bugzillaBugId.ToString()).Should(Be.Empty)");
        }
        ...
        {
            Context.BugzillaBugInfos.Where(x => x.Id == bugzillaBugId.ToString())
                .Should(Be.Not.Empty, "Context.BugzillaBugInfos.Where(x => x.Id == bugzillaBugId.ToString()).Should(Be.Not.Empty)");
        }
        ...
        {
            GetBugzillaBug(bugzillaBugId)
                .Component.Should(Be.EqualTo(component), "GetBugzillaBug(bugzillaBugId).Component.Should(Be.EqualTo(component))");
        }
        ...
        {
            GetBugzillaBug(bugzillaBugId)
                .Version.Should(Be.EqualTo(version), "GetBugzillaBug(bugzillaBugId).Version.Should(Be.EqualTo(version))");
        }
        ...
        {
            GetBugzillaBug(bugzillaBugId)
                .Platform.Should(Be.EqualTo(platform), "GetBugzillaBug(bugzillaBugId).Platform.Should(Be.EqualTo(platform))");
        }
        ...
        {
            GetBugzillaBug(bugzillaBugId)
                .Classification.Should(Be.EqualTo(classification),
        ...
        public void ShouldCheckCustomField(int bugzillaBugId, string fieldName, string fieldValue)
        {
            GetBugzillaBug(bugzillaBugId).CustomFields
                ...
                .Values.Should(Be.EquivalentTo(new[] { fieldValue }),
                    "GetBugzillaBug(bugzillaBugId).CustomFields.Single(f => f.Name == fieldName).Values.Should(Be.EquivalentTo(new[] {fieldValue}))");
        }
        ...
        public void ShouldCheckCustomField(int bugzillaBugId, string fieldName, string[] fieldValue)
        {
            GetBugzillaBug(bugzillaBugId).CustomFields
                ...
                .Should(Be.EquivalentTo(fieldValue),
                    "GetBugzillaBug(bugzillaBugId).CustomFields.Single(f => f.Name == fieldName).Values.Should(Be.EquivalentTo(fieldValue))");
        }
        ...
        private IEnumerable<BugzillaBugInfo> GetAllBugzillaBugs(IEnumerable<int> bugzillaBugIds)
        {
            ...
            foreach (var profile in ObjectFactory.GetInstance<IProfileCollectionReadonly>())
            {
                SetProfile(profile);
                tpBugIds.AddRange(
                    bugzillaBugIds.Select(x => Context.StorageRepository.GetTargetProcessBugId(x.ToString()))
                        .Where(x => x != null)
                        .Select(x => x.Value));
            }
