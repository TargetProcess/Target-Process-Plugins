		...
		return new Ext.XTemplate(
                '<span class="topTitle">Projects&nbsp;</span>',
                 '<a href="javascript:void(0)" class="projectSelectionLink">',
                    '<tpl for="selectedProjects">',
                        '<tpl if="xindex &lt; 3">{name:htmlEncode}</tpl>',
                    ...
                    '<span id="moreProjectsSpan" style="display:none;">',
                        '<tpl for="selectedProjects">',
                            '<tpl if="xindex &gt; 2">,&nbsp;</tpl>',
                            '<tpl if="xindex &gt; 2">{name:htmlEncode}</tpl>',
                        '</tpl>',
                    '</span>',
                 '</a>',
				...
				'<tpl if="projects.length &gt; 0">',
					'<div class="p-5" style="height: 18px; display: block;">',
						...
						'<a href="javascript:void(0)" class="unSelectAllProjects" style="display:none">deselect all</a>',
					'</div>',
                ...
                '<div class="projectSelectionScrolling"><table>',
                    '<tpl for="projects">',
                        ...
                        '</tpl>',
							'<tpl if="type==\'Project\'">',
                            	'<tpl if="abbreviation != &quot;&quot;"><div class="projectAbbr">{abbreviation:htmlEncode}</div></tpl><a onclick=\'projectContextService.setCurrent({id})\'>{name:htmlEncode}</a></td>',
							'</tpl>',
							'<tpl if="type==\'Program\'">',
								'<tpl if="childProjectIds.length == 0">',
									'<span style="color:#28428B; vertical-align:middle;">{name:htmlEncode}</span>',
								'</tpl>',
								'<tpl if="childProjectIds.length &gt; 0">',
                            		'<a onclick=\'projectContextService.setCurrent([{childProjectIds}])\'>{name:htmlEncode}</a>',
								'</tpl>',
								'</td>',
							'</tpl>',
                        '</tr>',
