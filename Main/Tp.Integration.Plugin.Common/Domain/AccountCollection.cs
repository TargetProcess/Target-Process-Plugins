...
namespace Tp.Integration.Plugin.Common.Domain
{
    ...
    internal sealed class AccountCollection : IAccountCollection
    {
        ...
        [ThreadStatic] private static AccountDomainObjectVersion _accountCurrentVersion;
        ...
        private Dictionary<AccountName, AccountDomainObjectVersion> Accounts
        {
            get
            {
                return _accountLatestVersions
                    ?? (_accountLatestVersions = _accountRepository.GetAll().ToDictionary(acc => acc.Name, AccountDomainObjectVersion.Root));
            }
        }
    }
}
