...
{
	[DebuggerDisplay("ProfileName = {_profileName.Value}, AccountName =  {_accountName.Value}, Initialized = {_initialized}, ProfileSettingsType = {_profileSettingsType}")]
	class ProfileDomainObject : StorageRepositoryDelegate, IProfile
	{
		...
		public object Settings
		{
			...
			set
			{
				...
				{
					throw new ApplicationException(string.Format("Profile '{0}' settings has wrong '{1}' type. Valid settings type is '{2}'", Name, value.GetType(), _profileSettingsType));
				}
				_settings = value;
			}
		}
