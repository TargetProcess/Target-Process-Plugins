...
namespace Tp.Integration.Plugin.Common.Mashup
{
    ...
    public class PluginMashup
    {
        ...
        public virtual bool IsValid => _filePaths.Select(GetFullPath).All(File.Exists);
            ...
            return
                _filePaths.Select(
                    x =>
                        ...
                        {
                            FileName = x,
                            ScriptContent = Convert.ToBase64String(File.ReadAllBytes(GetFullPath(x)))
                        }).
                    ToArray();
        }
        ...
        {
            get { return _filePaths.Any(x => x.Trim().ToLower().EndsWith(".cfg")); }
        }

        private string GetFullPath(string path)
        {
            var basePath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory,
                $"{DefaultPluginMashupRepository.PluginMashupDefaultPath}\\{MashupName}");
