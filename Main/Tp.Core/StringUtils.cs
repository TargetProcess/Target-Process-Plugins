...
namespace Tp.Core
{
    ...
    public static class StringUtils
    {
        ...
        {
            return (!string.IsNullOrEmpty(originString) && originString.Length > maxLength)
                ? originString.Substring(0, maxLength)
                : originString;
        }

        public static string ReplaceEndOfString(this string originString, string suffix, string newSuffix)
        {
            if (originString.EndsWith(suffix, StringComparison.InvariantCultureIgnoreCase)
                && !originString.EndsWith(newSuffix, StringComparison.InvariantCultureIgnoreCase))
            {
                originString = originString.Substring(0, originString.Length - suffix.Length) + newSuffix;
            }
            return originString;
        }
    }
