...
namespace System.Threading.Tasks.Schedulers
{
    ...
    {
        private static readonly ILog _log = LogManager.GetLogger(typeof(QueuedTaskScheduler));
        ...
        private void ThreadBasedDispatchLoop(Action threadInit, Action threadFinally)
        {
            ...
            try
            {
                ...
                try
                {
                    _log.Info("Starting thread loop...");
                    // If a thread abort occurs, we'll try to reset it and continue running.
                    while (true)
                    {
                        _log.Info("Starting thread loop iteration");
                        try
                        {
                            // For each task queued to the scheduler, try to execute it.
                            foreach (var task in _blockingTaskQueue.GetConsumingEnumerable(_disposeCancellation.Token))
                            {
                                _log.Debug("Processing next task...");
                        ...
                        {
                            _log.Info("Caught ThreadAbortException in scheduler loop, determining whether reset should be attempted...", ex);

                            var hasShutdownStarted = Environment.HasShutdownStarted;
                            var isFinalizingForUnload = AppDomain.CurrentDomain.IsFinalizingForUnload();
                            _log.Info($"hasShutdownStarted: {hasShutdownStarted}, isFinalizingForUnload: {isFinalizingForUnload}");

                            // If we received a thread abort, and that thread abort was due to shutting down
                            // or unloading, let it pass through.  Otherwise, reset the abort so we can
                            // continue processing work items.
                            if (!hasShutdownStarted && !isFinalizingForUnload)
                            {
                                _log.Info("Going to reset a caught thread abort in scheduler loop...");
                                Thread.ResetAbort();
                                _log.Info("Finished resetting a thread abort in scheduler loop");
                            }
                        }
                    }
                }
                ...
                {
                    _log.Info("Caught OperationCanceledException in scheduler loop", ex);
                }
            }
            finally
            {
        ...
        protected override IEnumerable<Task> GetScheduledTasks()
        {
            ...
            if (_targetScheduler == null)
            {
                ...
                return _blockingTaskQueue.WhereNotNull().ToList();
            }
            // otherwise get them from the non-blocking queue...
            else
            {
                return _nonthreadsafeTaskQueue.WhereNotNull().ToList();
            }
        }
