...
namespace Tp.Integration.Messages.ServiceBus.Serialization
{
    ...
    public class XmlDeserializer : IDisposable
    {
            ...
            if (!rootnode.Name.Equals(taglib.OBJECT_TAG))
            {
                ...
                if (rootnode == null)
                {
                    throw new ArgumentException(
                        "Invalid node. The specified node or its direct children do not contain a " + taglib.OBJECT_TAG + " tag.",
                        "XmlNode node");
                }
            }
        ...
        protected void SetCollectionValues(ICollection coll, XmlNode parentNode)
        {
            ...
            if (typeof(IList).IsAssignableFrom(coll.GetType()))
            {
                // IList
        ...
        protected Assembly GetAssembly(String assembly)
        {
            ...
            catch
            {
                try
                {
                    a = AppDomain.CurrentDomain.GetAssemblies().Single(y => y.GetName().Name == assembly);
                    assemblycache.Add(assembly, a);
                }
                catch
                {
        ...
        private object CreateInstance(ObjectInfo info)
        {
            try
            {
                ...
                if (type == null)
                {
                    throw new TypeNotFoundWhileDeserializationException(
                        $"Assembly or Type not found. Assembly: {info.Assembly}, Type: {info.Type}");
                }
        ...
        private TypeInfo TranslateTypeByKey(String key)
        {
            if (HasTypeDictionary)
            {
                ...
                {
                    TypeInfo ti = (TypeInfo) typedictionary[key];
