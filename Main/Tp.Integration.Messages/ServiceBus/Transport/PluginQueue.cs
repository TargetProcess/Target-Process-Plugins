...
namespace Tp.Integration.Messages.ServiceBus.Transport
{
    public class PluginQueue : IPluginQueue
    {
        ...
        public static bool TryDeleteQueue(string queueName, ILoggerContextSensitive log)
        {
            ...
            {
                log.Warn(LoggerContext.New(queueName), "Failed to delete queue {0}. Queue does not exist or no permissions".Fmt(queueName),
                    e);
                return false;
            }
        }
        ...
        {
            return _queue.Receive(messageQueueTransaction);
        }
    ...
    public interface IPluginQueue
    {
        ...
        Message Receive(MessageQueueTransaction messageQueueTransaction);
    }
}
