...
namespace Tp.Integration.Messages.ServiceBus.Transport.UiPriority
{
    ...
    public class MsmqUiPriorityTransport : IMsmqTransport
    {
        ...
        /// This value is only relevant when <see cref="TransactionMode"/> is set to <see cref="TransportTransactionMode.TransactionScope"/>.
        /// </summary>
            ...
            try
            {
                switch (TransactionMode)
                {
                    case TransportTransactionMode.QueueOnly:
                        ReceiveInMessageQueueTransaction(queue, ProcessMessage);
                        break;
                    case TransportTransactionMode.TransactionScope:
                        new TransactionWrapper().RunInTransaction(q => ReceiveFromQueue(q, ProcessMessage), queue, IsolationLevel, TransactionTimeout);
                        break;
                    default:
                        ReceiveFromQueue(queue, ProcessMessage);
                        break;
                }
        ...
        private void ReceiveFromQueue(IPluginQueue queue, Action<IPluginQueue, Message> processMessageAction)
        {
            var message = ReceiveMessageFromQueueAfterPeekWasSuccessful(queue);
            processMessageAction(queue, message);
        }

        private static void ReceiveInMessageQueueTransaction(IPluginQueue queue, Action<IPluginQueue, Message> processMessageAction)
        {
            TransportTransactionHelper.RunInQueueOnlyTransaction(mqTx =>
            {
                var message = queue.Receive(mqTx);
                processMessageAction(queue, message);
            });
        }

        private void ProcessMessage(IPluginQueue queue, Message m)
        {
            if (m == null)
            {
                return;
            ...
            {
                Logger.Error($"Message has failed the maximum number of {_retryCountPerMessageKeeper.MaxRetries} times allowed, ID={m.Id}.");
        ...
        {
            return TransactionMode == TransportTransactionMode.TransactionScope
