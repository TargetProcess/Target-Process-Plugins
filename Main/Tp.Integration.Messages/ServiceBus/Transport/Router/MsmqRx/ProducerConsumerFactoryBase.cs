...
{
	public class ProducerConsumerFactoryBase : IProducerConsumerFactory<MessageEx>
	{
		...
		public ProducerConsumerFactoryBase(ILoggerContextSensitive log, TimeSpan waitMessageTimeout, Func<MessageQueueTransactionType> sendTransactionType, MessageQueueTransactionType receiveTransactionType)
		{
		...
		private IEnumerable<IObservable<MessageEx>> GetMessageStream(string sourceName, bool isChild)
		{
			...
			Func<IObservable<Message>> recieveObservableFactory = Observable.FromAsyncPattern(
				(callback, obj) =>
					{
						_log.Info(LoggerContext.New(sourceName + "~source"), GetType().FullName + "GetMessageStream.BeginPeek");
		...
		protected ILoggerContextSensitive Log
		{
			get { return _log; }
		}
	}
